  ...
  useEffect(() => {
    ...
    const analyzeAndVibrate = async (uri) => {
      ...
      try {
        ...
        console.log('Checking thresholds:', {
          enableSpeed, actualWPM: data.wpm, target: userSettings.speedValue,
          enableVolume, actualVol: data.volume, target: userSettings.volumeValue,
          enableFiller, actualFiller: data.fillerCount, target: userSettings.fillerCount,
        });

        if (enableSpeed) {
          console.log(`⚡ Speed: ${data.wpm} vs threshold ${userSettings.speedValue}`);
          if (data.wpm > userSettings.speedValue) Vibration.vibrate(3000);
        }
        
        if (enableVolume) {
          console.log(`🔊 Volume: ${data.volume} vs threshold ${userSettings.volumeValue}`);
          if (data.volume > userSettings.volumeValue) Vibration.vibrate(3000);
        }
        
        if (enableFiller) {
          console.log(`💬 Filler Count: ${data.fillerCount} vs threshold ${userSettings.fillerCount}`);
          if (data.fillerCount > userSettings.fillerCount) Vibration.vibrate(3000);
        }
    ...
    const loopRecording = async () => {
      try {
        ...
        console.log('Preparing new recording...');
  ...
  useEffect(() => {
    ...
    const analyzeAndVibrate = async (uri) => {
      ...
      try {
        ...
        if (enableSpeed && data.wpm > userSettings.speedValue) Vibration.vibrate(3000);
        if (enableVolume && data.volume > userSettings.volumeValue) Vibration.vibrate(3000);
        if (enableFiller && data.fillerCount > userSettings.fillerCount) Vibration.vibrate(3000);
      } catch (err) {
        console.log('Analysis error:', err);
      }
    };
